...
// (3) 1. 곱마다 단을 계산하는건지? 2. 단 마다 곱을 계산하는건지? 
...
*             1             1                 첫번째 줄의 별 1개 출력        line은 1부터 5까지 1씩 증가 하면서 줄바꿈 (\n)
**            2             1 2               두번째 줄의 별 2개 출력        star는 1부터 line 까지 1씩 증가 하면서 "*" 출력
***           3             1 2 3             세번째 줄의 별 3개 출력
****          4             1 2 3 4           네번째 줄의 별 4개 출력
*****         5             1 2 3 4 5         다섯번째 줄의 별 5개 출력
찾은 패턴1 : line은 1부터 5까지 1씩증가 하면서 줄바꿈(\n) 출력
찾은 패턴2 : star는 1부터 line까지 1씩 증가 하면서 "*" 출력
*/
let output = ""; // 출력할 문자열 선언
// (1)
// for(let line = 1; line <= 5; line++){ // console.log() 함수는 1개당 줄바꿈1개 내장
//     output += "\n"
// } // 예상 : output = "\n\n\n\n\n";
...
// (3) 1. 줄마다 별 출력하는건지? [o (별을 출력하고 줄 바꿈)] 2. 별 마다 줄 출력하는건지? [x]

for (let line = 1; line <= 5; line++) {
    for (let star = 1; star <= line; star++) { // 총합계와 동일하게 문자열 추가/연결
        output += "*";
    }
    output += "\n"
}
...
// (1) continue;
for (let i = 1; i <= 5; i++) {
    if (i == 3) { continue; } // 만약에 i(반복변수)가 3일 때 반복문으로 이동 < 이하코드 실행 안됨 >
    console.log(i); // 1 2 4 5, 1부터 3제외한 5까지 출력된다. 이유 : 3일 때 continue; 만났기 때문
}//for end
